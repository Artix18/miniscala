...
let alloc_expr env decal exp = match exp with
  | Ecst(constant) -> PEcst(constant),decal
  | Ethis -> PEthis,decal
  | Enull -> PEnull,decal
  | Eaccess(lv) -> match lv with
  					| Lident(ident,_)-> PEaccess(Smap.find ident env), decal
  					| Laccess(exp, ident * (interv)
...
and alloc_decl decl ((env:(int Smap.t)), decal) nextL = match decl with
  | Dvar(var) -> let (_,ident,_,exp,_) = var in
  			      (*let ident = "this."^ident in *) (* mauvaise idée *)
  				  let nexp, nDecal = alloc_expr env decal exp in
  				  let decal = decal + 8 in
  				  let nEnv = Smap.add ident (-decal) env in
  				  let (rDeco, rMax) = alloc_decl_l nextL (nEnv, decal) in
  				  (PDvar(-decal, nexp))::rDeco, (max nDecal rMax)
  | Dmeth(methode) -> let (over,ident,ptl,pl,typ,locd_expr,interv) = methode in
  					  let nEnv,nxt=List.fold_left (fun (ev,nxt) (id,_) -> (*let id="this."^id in *) let nxt = nxt + 8 in (Smap.add id nxt ev, nxt)) (env,8) pl
  					  in
